...
class BtcSignatureCallback extends BaseOracle {
    ...
    async refundTxSignCallback(roleEnum, settlementId, signedRawTransaction, pubKey = '') {
        ...
        const settlementInfo = await _self.readData(settlementInfoKey);
        const hashLockTx = await _self.readData(submitHashLockTxKey);
        ...
        const {chain, currency, confirmThreshold, lockId: lid} = _self.formatSettlement(roleEnum, settlementInfo);
        let lockId = hashLockTx ? hashLockTx.lockId : lid;
