...
class MqttService {
    ...
    async onError(error) {
        ...
        if (!mqOptions || !mqOptions.hasOwnProperty('username')) {
            return this.statusNotice(MQTT_CONNECT_STATUS.FAILED, `reset mq options no hash property 'username', kofoId=${this.kofoId}`);
        }
        this.mqOptions = mqOptions;
        this.init();
    ...
    onConnect() {
        this.mqtt.subscribe(this.kofoId, (err) => {
            if (err) {
                return this.statusNotice(MQTT_CONNECT_STATUS.FAILED, `subscribe kofoId: ${this.kofoId} error, message=${err.message}`);
            }
     ...
     */
    onClose() {
        this.statusNotice(MQTT_CONNECT_STATUS.FAILED, `mqtt server close, kofoId=${this.kofoId}`);
    }
     ...
     */
    onReconnect() {
        this.statusNotice(MQTT_CONNECT_STATUS.SUCCESS, `mqtt server reconnect, kofoId=${this.kofoId}`);
    }
