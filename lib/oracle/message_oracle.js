...
class Oracle extends BaseOracle{
    ...
    async submitHashLockTxHandler(message) {
        ...
        if (!settlementInfo) {
            throw new TypeError(`${roleEnum} ${message.chain} submit hash lock transaction error, not read cache data settlementInfo=${!!settlementInfo} flow settlementInfoKey=${settlementInfoKey}`);
        }
        settlementInfo = Object.assign(settlementInfo, _.omit(message, ['type', 'hValue', 'makerLockId']));
        const {chain, currency, amount, pubKey: sender, lockTime, oppositeCounterChainPubKey: receiver, oppositeFee: fee, adminPubKey, addressType} = this.formatSettlement(roleEnum, settlementInfo);
    ...
    async submitWithdrawTxHandler(message) {
        ...
        let {counterChainPubKey: sender, oppositeLockId: lockId, fee, oppositeAdminPubKey: adminPubKey, counterChainAddressType} = _self.formatSettlement(roleEnum, message);
    ...
    async submitRefundTxHandler(message) {
        ...
        const {pubKey: sender, adminPubKey, fee, addressType} = _self.formatSettlement(roleEnum, settlementInfo);
        const _statusNotice_ = function (type, extra) {
            let msg = {settlementId, roleEnum, chain, currency, lockId, sender};
            _self.statusNotice(type, Object.assign(msg, extra));
        };
