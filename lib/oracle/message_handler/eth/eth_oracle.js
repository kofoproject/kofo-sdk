...
class EthOracle extends Oracle {
    ...
    async submitWithdrawTxHandler(message) {
        ...
        if (!lockTx || !settlementInfo) {
            throw new TypeError(`Not read cache data ETH ${roleEnum} lockTx=${!!lockTx} settlementInfo=${!!settlementInfo} flow submitHashLockTxKey=${submitHashLockTxKey} and settlementInfoKey=${settlementInfoKey}`);
        }
