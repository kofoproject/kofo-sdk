...
/**
 * Provides TRX settlement order message handler
 */
class TrxOracle extends Oracle {
    constructor() {
    ...
    async submitHashLockTxHandler(message) {
        ...
        if (!settlementInfo) {
            throw new TypeError(`Not read cache data TRX ${roleEnum} settlement info flow ${settlementInfoKey}`);
        }
    ...
    async submitWithdrawTxHandler(message) {
        ...
        if (!lockTx || !settlementInfo) {
            throw new TypeError(`Not read cache data TRX ${roleEnum} lockTx=${!!lockTx} settlementInfo=${!!settlementInfo} flow submitHashLockTxKey=${submitHashLockTxKey} and settlementInfoKey=${settlementInfoKey}`);
        }
