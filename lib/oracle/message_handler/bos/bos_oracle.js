...
class BosOracle extends Oracle {
    ...
    async submitHashLockTxHandler(message) {
        ...
        if (!settlementInfo) {
            throw new TypeError(`Not read cache data BOS ${roleEnum} settlement info flow ${settlementInfoKey}`);
        }
    ...
    async submitWithdrawTxHandler(message) {
        ...
        if (!lockTx || !settlementInfo) {
            throw new TypeError(`Not read cache data BOS ${roleEnum} lockTx=${!!lockTx} settlementInfo=${!!settlementInfo} flow submitHashLockTxKey=${submitHashLockTxKey} and settlementInfoKey=${settlementInfoKey}`);
        }
