...
/**
 * Provides MEETONE with Hash lock, Withdraw, Refund, Approve signature callback handler
 */
class MeetOneOracle extends Oracle {
    constructor() {
    ...
    async submitHashLockTxHandler(message) {
        ...
        if (!settlementInfo) {
            throw new TypeError(`Not read cache data MEETONE ${roleEnum} settlement info flow ${settlementInfoKey}`);
        }
    ...
    async submitWithdrawTxHandler(message) {
        ...
        if (!lockTx || !settlementInfo) {
            throw new TypeError(`Not read cache data MEETONE ${roleEnum} lockTx=${!!lockTx} settlementInfo=${!!settlementInfo} flow submitHashLockTxKey=${submitHashLockTxKey} and settlementInfoKey=${settlementInfoKey}`);
        }
