...
/**
 * Provides hpb and Erc20 settlement order message handler
 */
class HPBOracle extends Oracle {
    constructor() {
    ...
    async submitHashLockTxHandler(message) {
        ...
        if (!settlementInfo) {
            throw new TypeError(`Not read cache data HPB ${roleEnum} settlement info flow ${settlementInfoKey}`);
        }
    ...
    async submitWithdrawTxHandler(message) {
        ...
        if (!lockTx || !settlementInfo) {
            throw new TypeError(`Not read cache data HPB ${roleEnum} lockTx=${!!lockTx} settlementInfo=${!!settlementInfo} flow submitHashLockTxKey=${submitHashLockTxKey} and settlementInfoKey=${settlementInfoKey}`);
        }
