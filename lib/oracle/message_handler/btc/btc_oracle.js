...
class BtcOracle extends Oracle {
    ...
    async submitHashLockTxHandler(message) {
        ...
        const {chain, currency, amount, pubKey, lockTime, oppositeCounterChainPubKey, oppositeFee: fee, adminPubKey, addressType} = _self.formatSettlement(roleEnum, settlementInfo);
    ...
    async submitWithdrawTxHandler(message) {
        ...
        if (!lockTx || !settlementInfo) {
            throw new TypeError(`Not read cache data BTC ${roleEnum} lockTx=${!!lockTx} settlementInfo=${!!settlementInfo} flow submitHashLockTxKey=${submitHashLockTxKey} and settlementInfoKey=${settlementInfoKey}`);
        }
        ...
        let {counterChainPubKey: pubKey, oppositeLockId: lockId, oppositeFee, fee, oppositeAdminPubKey: adminPubKey, counterChainAddressType} = _self.formatSettlement(roleEnum, message);
            ...
            try {
                ...
                if (!hasWithdrawTx) {
                    ...
                    withdrawTx = await gatewayProvider.createWithdrawTx({
                        ...
                        bizFee: fee,
                        fee: '',
                        isSegWitAddress: !!counterChainAddressType
                    });
    ...
    async submitRefundTxHandler(message) {
        ...
        const {chain, currency, pubKey, adminPubKey, fee, addressType} = _self.formatSettlement(roleEnum, settlementInfo);
        const _statusNotice_ = function (type, extra) {
            let msg = {settlementId, roleEnum, chain, currency, lockId, pubKey};
            _self.statusNotice(type, Object.assign(msg, extra));
        };
